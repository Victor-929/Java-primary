...
public class ProxyUtil {
    ...
    public static StartService createProxy(Start s){
        /**
         * 参数一：用于指定用那个类加载器去加载生成的代理类
         * 参数二：指定代理类需要实现的接口:明星类实现了哪些接口，代理类就实现哪些接口
         * 参数三：用于指定代理类需要如何去代理（代理要做的事情）
         */
        StartService proxy = (StartService) Proxy.newProxyInstance(ProxyUtil.class.getClassLoader(),
                s.getClass().getInterfaces(), new InvocationHandler() {
                    @Override
                    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                        //用来声明代理类要做的事情
                        //参数一：Proxy接受到代理对象本身（暂时用处不大）
